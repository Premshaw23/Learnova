...
const AttendanceTrendsChart = dynamic(
    () =>
    import ("@/components/charts/AttendanceTrendsChart"), { ssr: false, loading: () => < ChartSkeleton variant = "chart" / > },
);
const EngagementChart = dynamic(
    () =>
    import ("@/components/charts/EngagementChart"), { ssr: false, loading: () => < ChartSkeleton variant = "doughnut" / > },
);
...
const TeacherDashboard = () => {
    ...
    const renderDashboard = () => ( <
        div className = "space-y-8" > { /* Passcode Generation Section */ } {
            attendanceWindow && ( <
                ...
                /div> {
                    passcodeExpiresAt && ( <
                        ...
                        div className = "text-white font-semibold" > { new Date(passcodeExpiresAt).toLocaleTimeString() } <
                        /div> <
                        /div>
                    )
                } <
...

                {
                    !passcodeGenerated ? ( <
                        ...
                        span > { passcodeLoading ? "Generating..." : "Generate Attendance Passcode" } < /span> {!passcodeLoading && < Sparkles className = "w-5 h-5" / > } <
                        /span> <
                        /button>
                    ) : ( <
                        ...
                        Active Passcode <
                        /div> <
                        div className = "text-2xl font-mono text-white font-bold tracking-wider" > { currentPasscode } <
                        /div> {
                            passcodeExpiresAt && ( <
                        ...
                        /div> <
                        /div> <
                        button onClick = { closeAttendanceWindow }
                        disabled = { passcodeLoading }
        ...
        div className = "lg:col-span-2 space-y-8" > { /* Attendance Overview */ } <
        ...
        /h2> <
        button aria - label = "Refresh attendance"
        ...
        <
        div className = "text-2xl font-bold text-blue-400" > { attendanceStats.totalStudents } <
        /div> <
        ...
        <
        div className = "text-2xl font-bold text-green-400" > { attendanceStats.presentToday } <
        /div> <
        ...
        <
        div className = "text-2xl font-bold text-red-400" > { attendanceStats.absentToday } <
        /div> <
        ...
        <
        div className = "text-2xl font-bold text-yellow-400" > { attendanceStats.lateToday } <
        /div> <
        ...
        div className = "space-y-2" > {
            studentAttendanceData.map((student) => ( <
                ...
                div >
                <
                div className = "text-white font-medium" > { student.name } <
                /div> <
                div className = "text-gray-400 text-sm" > { student.rollNo } <
                /div> <
                /div> <
                /div>
                ...
                div className = "text-gray-400 text-sm mt-1" > {
                    student.status !== "absent" && ( <
                        span > { student.time }({ student.confidence } % ) <
                        /span>
                    )
                } <
                /div> <
        ...
        { /* Exception Requests */ } <
        ExceptionRequestsList exceptionRequests = { exceptionRequests }
        ...
        /> <
        /div> { /* Sidebar */ } <
        div className = "space-y-8" > { /* Today's Schedule */ } <
        div className = "bg-black/40 backdrop-blur-xl rounded-2xl border border-white/10 p-6" >
        <
            ...
            todayClasses.length > 0 ? ( <
                div className = "space-y-3" > {
                    todayClasses.map((cls, index) => ( <
                        ...
                        <
                        div className = "text-white font-medium" > { cls.subject } <
                        /div> <
                        div className = "text-sm text-gray-400" > { cls.time } < /div> <
                        /div> <
                        div className = "text-sm text-gray-400 mb-2" > { cls.semester } - Section { cls.section } <
                        /div> <
                        div className = "flex items-center justify-between" >
                        <
                        ...
                        <
                        span className = "text-xs text-blue-400" > { cls.students } <
                        /span> <
                        /div> <
                        /div> <
                        /div>
                ...
                <
                p className = "text-gray-400" > No classes scheduled
                for today < /p> <
                /div>
            )
        } <
        ...
        { /* Passcode Modal */ } <
        AttendancePasscodeModal showPasscodeModal = { showPasscodeModal }
    ...
    const renderAnalytics = () => ( <
        ...
        <
        div className = "grid grid-cols-1 lg:grid-cols-2 gap-8" > { /* Attendance Trends */ } <
        div className = "bg-black/40 backdrop-blur-xl rounded-2xl border border-white/10 p-6" >
        <
    ...
    const renderSchedule = () => ( <
        ...
        div className = "grid grid-cols-1 md:grid-cols-7 gap-4" > {
            Object.entries(weeklySchedule).map(([day, classes]) => ( <
                ...
                className = "space-y-3" >
                <
                h3 className = "text-lg font-bold text-white text-center" > { day } <
                /h3> {
                    classes.map((cls, index) => ( <
                        div key = { index }
                        className = "bg-gray-800/50 rounded-lg p-3 border border-gray-700/50" >
                        <
                        div className = "text-sm font-medium text-white" > { cls.subject } <
                        /div> <
                        div className = "text-xs text-gray-400" > { cls.time } < /div> <
                        div className = "text-xs text-accent" > { cls.room } < /div> <
                        div className = "text-xs text-blue-400" > { cls.students }
                        students <
                        /div> <
                        /div>
                    ))
    ...
    return ( <
        div className = "min-h-screen bg-gradient-to-br from-black via-gray-900 to-black relative overflow-hidden" > { /* Premium Navbar */ } <
        Navbar / > { /* Animated Gradient Backgrounds */ } <
        ...
        <
        div className = "bg-black/20 backdrop-blur-xl rounded-2xl border border-white/10 p-6 shadow-2xl" > { /* Main Header Row */ } <
        div className = "flex flex-col md:flex-row md:items-center justify-between gap-4" > { /* Left - Teacher Profile */ } <
        ...
        size = "md"
        shape = "rounded"
        showStatusDot name = { displayName }
        initials = { initials }
        ...
        div >
        <
        h1 className = "text-xl md:text-2xl font-bold bg-gradient-to-r from-white to-accent bg-clip-text text-transparent" > { displayName } <
        /h1> <
        ...
        { /* Right - Time & Status */ } <
        div className = "flex items-center gap-6" > { /* Current Time */ } <
        div className = "text-right" >
        <
      ...
      ` } < /style> { /* Main Content */ } <
        div className = "relative z-10 container mx-auto px-6 py-8" > { activeTab === "dashboard" && renderDashboard() } { activeTab === "analytics" && renderAnalytics() } { activeTab === "schedule" && renderSchedule() } <
        /div> <
        /div>
    );
};

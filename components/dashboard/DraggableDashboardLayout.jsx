...
export default function DraggableDashboardLayout({
  children,
  defaultLayout,
  layoutKey,
}) {
  ...
  if (!mounted)
    return (
      <div className="animate-pulse h-96 w-full bg-card/10 rounded-2xl"></div>
    );
    ...
    >
      {React.Children.map(children, (child) => {
        ...
        return (
          <div
            key={child.key}
            className="relative group bg-card/40 dark:bg-black/40 backdrop-blur-xl rounded-2xl border border-border dark:border-white/10 flex flex-col overflow-hidden h-full"
          >

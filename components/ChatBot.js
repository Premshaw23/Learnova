...
export default function LearnovaChatbot() {
  ...
  if (!isOpen) {
    return (
      <div
        className={`fixed z-50 transition-all duration-300 right-4 md:right-6 ${
          isScrolling
            ? "bottom-16 opacity-40 scale-90 md:bottom-6 md:opacity-100 md:scale-100"
            : "bottom-24 md:bottom-6 opacity-100 scale-100"
        }`}
      >
        <button
  ...
  return (
    <div
      className={`fixed z-50 flex flex-col ${
        themeTokens.bg
      } shadow-2xl transition-all duration-300 border ${themeTokens.border} ${
        isMinimized
                  ...
                  >
                    <div
                      className={`p-2 rounded-xl shrink-0 ${
                        msg.isBot
                          ? themeTokens.botAvatar
                          : themeTokens.userAvatar
                      }`}
                    >
                      {msg.isBot ? <Bot size={16} /> : <User size={16} />}
                    </div>
                    <div
                      className={`max-w-[80%] rounded-2xl px-3.5 py-2 text-sm shadow-sm transition-all duration-200 ${
                        msg.isBot ? themeTokens.botMsg : themeTokens.userMsg
                      }`}
                    >
                      {msg.isBot ? (

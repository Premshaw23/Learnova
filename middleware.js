...
function getCachedRole(token) {
  const entry = roleCache.get(token);
  if (entry && Date.now() - entry.timestamp < ROLE_CACHE_TTL) return entry.role;
  if (entry) roleCache.delete(token);
  return null;
}

function setCachedRole(token, role) {
  roleCache.set(token, { role, timestamp: Date.now() });
}
  ...
  if (authToken) {
    ...
    if (payload) {
      ...
      if (payload.role) {
        ...
        const cachedRole = getCachedRole(authToken);
        if (cachedRole) {
          userRole = cachedRole;
        } else {
          try {
            ...
            if (res.ok) {
              ...
              if (userRole) setCachedRole(authToken, userRole);
            }
          } catch (err) {

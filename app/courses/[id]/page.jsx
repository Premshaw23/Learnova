...
export default function CourseDetailPage() {
  ...
  const handleSeek = (seconds) => {
    if (videoRef.current) {
      ...
      toast.success(
        `Jumped to ${Math.floor(seconds / 60)}:${String(seconds % 60).padStart(
          2,
          "0"
        )}`
      );
        ...
        >
          ...
          <div
            className={`flex flex-col ${
              isPodActive ? "lg:flex-row gap-6 items-start" : "w-full"
            }`}
          >
            {/* 3. Left Side Content Area */}
            <div
              className={`transition-all duration-300 ${
                isPodActive ? "w-full lg:flex-1" : "w-full"
              }`}
            >
                    ...
                    {mockVideoAIProperties.conceptMap.map((segment, index) => {
                      ...
                      return (
                        <div
                          ...
                          } h-full border-r border-zinc-950/40 cursor-pointer transition-all hover:brightness-125`}
                              ...
                              <span
                                className={`text-sm transition-colors ${
                                  lesson.completed
                                    ...
                                    : "text-zinc-300 group-hover/lesson:text-indigo-400"
                                }`}
                              >
                                {lesson.title}
                              </span>

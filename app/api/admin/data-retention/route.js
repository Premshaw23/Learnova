...
export async function POST(req) {
  try {
    ...
    if (decodedToken.role !== "admin") {
      return NextResponse.json(
        { error: "Forbidden: Only Super Admins can update policies" },
        { status: 403 }
      );
    }
    ...
    if (
      typeof attendanceRetentionMonths !== "number" ||
      typeof biometricPurgeMonths !== "number"
    ) {
      return NextResponse.json(
        { error: "Invalid data types for months" },
        { status: 400 }
      );
    }

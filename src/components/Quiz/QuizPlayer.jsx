    ...
    return (
      <div className="quiz-result-card p-6 bg-white rounded-lg shadow-md">
        ...
        <div className="result-stats flex flex-col gap-2">
          <p className="text-lg">
            Score: <span className="font-semibold">{result.score}</span> / {result.totalQuestions}
          </p>
          <p className="text-lg">
            Percentage: <span className="font-semibold">{result.percentage}%</span>
          </p>
              ...
              {q.options && q.options.map((opt) => (
                <label
                  ...
                  className={`flex items-center gap-3 p-3 rounded border cursor-pointer hover:bg-gray-50 ${
                    selectedAnswers[q.id] === opt ? 'border-blue-500 bg-blue-50' : 'border-gray-200'
                  }`}
                >
                  <input
                    type="radio"

    ...
    if (isDuplicate) {
      ...
      if (existingOp) {
        ...
        if (existingOp.status === "failed") {
          // Atomically transition status from failed to in_progress to allow retry
          const result = await db.collection("pending_operations").updateOne(
          ...
          if (result.matchedCount === 0) {
            const { AppError } = await import("@/lib/errors");
            throw new AppError("Operation state changed during retry.", 409);
          }
        }
      }
    } else {

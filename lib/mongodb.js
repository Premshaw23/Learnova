...
const options = {
  ...
  minPoolSize: 0,               // Tuned for serverless (no minimum connections)
  maxIdleTimeMS: 60000,         // Explicitly reap stale zombie connections after 60s
  serverSelectionTimeoutMS: 5000, 
  connectTimeoutMS: 5000,       // Fast fail on initial connection timeouts
...
export async function connectDb() {
  try {
    ...
    const connectedClient = await getCachedClientPromise();
...
// ============================================================================
// 📡 DEDICATED SSE CONNECTION POOL (Lazily Initialized)
// ============================================================================
const sseOptions = {
  maxPoolSize: 5,               // Reduced from 30 to limit database load in serverless
  minPoolSize: 0,               // No minimum idle connections
  serverSelectionTimeoutMS: 5000,
  ...
  if (!sseClientPromise) {
    ...
    if (process.env.NODE_ENV === "development") {
      if (!global._mongoSseClientPromise) {
        global._mongoSseClientPromise = initSseClient();
      }
      sseClientPromise = global._mongoSseClientPromise;
    } else {
      sseClientPromise = initSseClient();
    }
  }
...
export async function connectDbForSSE() {
  try {
    const dbName = process.env.MONGODB_DB;
    const promise = getSseClientPromise();
...
export function getDbMetrics() {
   return {
     activePool: (mongoClientPromise || cachedPromise) ? "connected" : "disconnected",
     ...metrics
   };
}

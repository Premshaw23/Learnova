...
async function ensureIndexes(db) {
  try {
    ...
    if (logger?.error) logger.error("[DB Manager] Index creation failed", err);
  }
}
  ...
  if (!sseClientPromise) {
    ...
    if (process.env.NODE_ENV === "development") {
      if (!global._mongoSseClientPromise) {
        global._mongoSseClientPromise = new MongoClient(uri, sseOptions).connect();
      }
      sseClientPromise = global._mongoSseClientPromise;
    } else {
      sseClientPromise = new MongoClient(uri, sseOptions).connect();
    }
  }
...
export function getDbMetrics() {
   return {
     activePool: cachedPromise ? "connected" : "disconnected",
     ...metrics
   };
}

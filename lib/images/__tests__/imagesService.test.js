...
describe("imagesService helpers", () => {
  ...
  test("updateUserImageInDb updates image and deletes faceDescriptor by default", async () => {
    const usersUpdateOne = vi.fn().mockResolvedValue({ matchedCount: 1 });
    const faceDescriptorsDeleteOne = vi.fn().mockResolvedValue({ deletedCount: 1 });

    connectDb.mockResolvedValue({
      collection: vi.fn().mockImplementation((name) => {
        if (name === "face_descriptors") {
          return { deleteOne: faceDescriptorsDeleteOne };
        }
    ...
    expect(faceDescriptorsDeleteOne).toHaveBeenCalledWith({ firebaseUid: "firebase-uid-123" });
  ...
  test("updateUserImageInDb updates image and upserts faceDescriptor when provided", async () => {
    const usersUpdateOne = vi.fn().mockResolvedValue({ matchedCount: 1 });
    const faceDescriptorsUpdateOne = vi.fn().mockResolvedValue({ matchedCount: 1 });

    connectDb.mockResolvedValue({
      collection: vi.fn().mockImplementation((name) => {
        if (name === "face_descriptors") {
          return { updateOne: faceDescriptorsUpdateOne };
        }
        return { updateOne: usersUpdateOne };
      }),
    });
    ...
    expect(faceDescriptorsUpdateOne).toHaveBeenCalledWith(
      { firebaseUid: "firebase-uid-123" },
      {
        $set: {
          firebaseUid: "firebase-uid-123",
  ...
  test("updateUserImageInDb throws when no Mongo profile matches firebase uid", async () => {
    const usersUpdateOne = vi.fn().mockResolvedValue({ matchedCount: 0 });
    const faceDescriptorsDeleteOne = vi.fn().mockResolvedValue({ deletedCount: 0 });

...
export const CSRF_PROTECTED_PATHS = ["/api/", "/auth/"];
...
export function getCsrfCookieOptions() {
  ...
  // If you truly need secure cookies for local development, set:
...
export function getClientCsrfToken() {
  ...
  for (const cookiePart of cookieParts) {
    if (cookiePart.startsWith(cookiePrefix)) {
      _csrfTokenCache = decodeURIComponent(cookiePart.slice(cookiePrefix.length));
...
export function isSameOriginApiUrl(url) {
  ...
  try {
    ...
    return isSameOrigin && isProtected;
  } catch {
    return false;
  }
}

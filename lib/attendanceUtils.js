...
/**
 * Project the two concrete numbers a student actually asks about: how many
 * upcoming classes they can safely skip while staying at or above the
 * threshold, and how many they must attend in a row to climb back to it.
 * @param {number} attended - Classes attended so far
 * @param {number} total - Total classes held so far
 * @param {number} threshold - Minimum required percentage (default 75)
 * @returns {{ percentage: number, status: string, canSkip: number, mustAttend: number }}
 */
export function getAttendanceProjection(attended, total, threshold = 75) {
  const totalHeld = Number(total) || 0;
  if (totalHeld <= 0) {
    return { percentage: 0, status: "critical", canSkip: 0, mustAttend: 0 };
  }

  const attendedClamped = Math.max(
    0,
    Math.min(Number(attended) || 0, totalHeld)
  );
  const percentage = Math.round((attendedClamped / totalHeld) * 100);
  const status = getAttendanceStatus(percentage);

  // Largest k such that attended / (total + k) >= threshold / 100.
  const canSkip = Math.max(
    0,
    Math.floor((attendedClamped * 100) / threshold - totalHeld)
  );

  // Smallest k such that (attended + k) / (total + k) >= threshold / 100.
  const remainingGap = 100 - threshold;
  const mustAttend =
    remainingGap <= 0
      ...
      : Math.max(
          0,
          Math.ceil(
            (threshold * totalHeld - attendedClamped * 100) / remainingGap
          )
        );

  return { percentage, status, canSkip, mustAttend };
}

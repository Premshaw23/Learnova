...
export async function registerBackgroundSync() {
  if (typeof window !== "undefined" && "serviceWorker" in navigator && "SyncManager" in window) {
    try {
      ...
      syncAttendanceQueue().catch((err) =>
        logger.error("[Sync] Manual flush fallback failed:", { error: err })
      );
    }
  } else {
    syncAttendanceQueue().catch((err) =>
...
/**
 * Registers the network-reconnection listener and returns a cleanup function.
 * Prevents listener leaks by providing explicit teardown.
 */
export function setupNetworkListener() {
  ...
  const handler = () => {
    ...
    syncAttendanceQueue().catch((err) =>
      logger.error("[Sync] Network sync failed:", { error: err })
    );
  };
...
// Auto-setup on import; exported cleanup can be called during HMR / unmount.
/** @type {(() => void) | undefined} */
let _cleanupNetwork;
if (typeof window !== "undefined") {
  _cleanupNetwork = setupNetworkListener();
  if (import.meta.hot) {
    import.meta.hot.dispose(() => {
      if (_cleanupNetwork) _cleanupNetwork();
    });
  }
}
}

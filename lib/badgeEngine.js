...
export function calculateConsecutiveAttendance(attendanceRecords = []) {
  ...
  // Filter out weekends, parse dates, and remove duplicate dates
  const sorted = [];
  ...
  // If today is a weekend, start looking from the last weekday (Friday)
  while (currentDate.getDay() === 0 || currentDate.getDay() === 6) {
    currentDate.setDate(currentDate.getDate() - 1);
  }

  const latestRecordStr = sorted[0].toISOString().split("T")[0];
  const todayStr = currentDate.toISOString().split("T")[0];
  
  const prevWeekday = new Date(currentDate);
  do {
  ...
  let matchDate;
  if (latestRecordStr === todayStr) {
    matchDate = currentDate;
  } else if (latestRecordStr === prevWeekdayStr) {
    matchDate = prevWeekday;
  } else {
    return 0;
  }

  let consecutiveDays = 0;
  for (const recordDate of sorted) {
    if (recordDate.getTime() === matchDate.getTime()) {
      consecutiveDays++;
      // Move matchDate to the previous weekday
      do {
        matchDate.setDate(matchDate.getDate() - 1);
      } while (matchDate.getDay() === 0 || matchDate.getDay() === 6);
    } else {
      break;
    }
  }

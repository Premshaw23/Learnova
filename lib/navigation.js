import {
  Activity,
  ArrowRight,
  BookOpen,
  BrainCircuit,
  Calendar,
  HeartPulse,
  Home,
  LayoutDashboard,
  Mail,
  MessageSquareWarning,
  Settings,
  Sparkles,
  User,
  UserCheck,
  Bell,
  LogIn,
  Compass,
  GraduationCap,
  ClipboardList,
  Beaker,
  MessageCircle,
  Box,
  Timer,
  BarChart,
  FolderGit,
  Award,
  Users,
} from "lucide-react";

export const NAVIGATION_ITEMS = [
  { href: "/", label: "Home", icon: Home },
  { href: "/about", label: "About", icon: BookOpen },
  { href: "/wellness", label: "Wellness", icon: HeartPulse },
  { href: "/productivity", label: "Focus", icon: Sparkles },
  { href: "/activity", label: "Activities", icon: Activity },
  { href: "/calendar", label: "Calendar", icon: Calendar },
];

export const DASHBOARD_ROUTES = [
  "/student/dashboard",
  "/teacher/dashboard",
  "/institute/dashboard",
  "/admin/dashboard",
  "/parent/dashboard",
];

export function normalizePath(pathname) {
  if (typeof pathname !== "string") return "";
  const base = pathname.split(/[?#]/)[0].trim();
  if (!base) return "";
  return base.replace(/\/+$/, "").replace(/^\/?/, "/");
}

function getKnownRouteMatch(pathname) {
  const path = normalizePath(pathname);
  if (!path) return null;

  const exactMatch = NAVIGATION_ITEMS.find((item) => {
    if (item.href === "/") {
      return path === "/";
    }

    return path === item.href || path.startsWith(`${item.href}/`);
  });

  if (exactMatch) return exactMatch;

  if (
    DASHBOARD_ROUTES.some(
      (route) => path === route || path.startsWith(`${route}/`)
    )
  ) {
    return { href: path, label: "Dashboard", icon: LayoutDashboard };
  }

  if (path === "/auth" || path.startsWith("/auth/")) {
    return { href: path, label: "Sign In", icon: LogIn };
  }

  if (path === "/profile" || path.startsWith("/profile/")) {
    return { href: path, label: "Profile", icon: User };
  }

  if (path === "/settings" || path.startsWith("/settings/")) {
    return { href: path, label: "Settings", icon: Settings };
  }

  if (path === "/attendance" || path.startsWith("/attendance/")) {
    return { href: path, label: "Attendance", icon: UserCheck };
  }

  if (path === "/notices" || path.startsWith("/notices/")) {
    return { href: path, label: "Notice Board", icon: Bell };
  }

  return null;
}

export function isRouteActive(pathname, href) {
  const path = normalizePath(pathname);
  if (!path || !href) return false;
  if (href === "/") return path === "/";
  return path === href || path.startsWith(`${href}/`);
}

export function isDashboardRoute(pathname) {
  const path = normalizePath(pathname);
  return DASHBOARD_ROUTES.some(
    (route) => path === route || path.startsWith(`${route}/`)
  );
}

export function getDashboardSidebarItems(role) {
  const dashboardHref = getDashboardLink(role);
  const primaryItems = [
    {
      href: dashboardHref,
      label: "Dashboard",
      icon: LayoutDashboard,
      key: "dashboard",
    },
    { href: "/attendance", label: "Attendance", icon: UserCheck, key: "attendance" },
    { href: "/notices", label: "Notices", icon: Bell, key: "notices" },
  ];

  if (role === 'teacher') {
    primaryItems.push({ href: "/teacher/meetings", label: "Meetings", icon: Calendar, key: "meetings" });
  } else if (role === 'parent') {
    primaryItems.push({ href: "/parent/meetings", label: "Meetings", icon: Calendar, key: "meetings" });
  }

  return {
    primary: [
      {
        href: dashboardHref,
        label: "Dashboard",
        icon: LayoutDashboard,
        key: "dashboard",
      },
      ...(['teacher', 'parent'].includes(role) ? [
        { href: `/${role}/messages`, label: "Messages", icon: MessageCircle, key: "messages" }
      ] : []),
      ...(['teacher'].includes(role) ? [
        { href: `/teacher/behavior`, label: "Behavior Insights", icon: Activity, key: "behavior" }
      ] : []),
      ...(['student'].includes(role) ? [
        { href: `/student/planner`, label: "Study Planner", icon: Calendar, key: "planner" },
        { href: `/student/portfolio`, label: "My Portfolio", icon: FolderGit, key: "portfolio" },
        { href: `/student/achievements`, label: "Badges & Milestones", icon: Award, key: "achievements" },
        { href: `/student/tutoring`, label: "Peer Tutoring", icon: Users, key: "tutoring" },
        { href: `/student/career`, label: "Career Paths", icon: Compass, key: "career" },
        { href: `/student/flashcards`, label: "AI Flashcards", icon: Brain, key: "flashcards" },
        { href: `/student/peer-reviews`, label: "Peer Reviews", icon: ClipboardList, key: "peer-reviews" },
        { href: `/student/lab`, label: "STEM Lab", icon: Beaker, key: "lab" }
      ] : []),
      { href: `/${role}/assignments`, label: "Assignments", icon: BookOpen, key: "assignments" },
      { href: "/attendance", label: "Attendance", icon: UserCheck, key: "attendance" },
      { href: "/notices", label: "Notices", icon: Bell, key: "notices" },
      { href: `/${role}/meetings`, label: "Meetings", icon: Calendar, key: "meetings" },
      ...(['teacher', 'admin'].includes(role) ? [
        { href: `/${role}/polls`, label: "Live Polls", icon: BarChart, key: "polls" }
      ] : []),
    ],
    explore: NAVIGATION_ITEMS,
    account: [
      { href: "/profile", label: "Profile", icon: User, key: "profile" },
      { href: "/settings", label: "Settings", icon: Settings, key: "settings" },
    ],
  };
}

export function getDashboardLink(role) {
  switch (role) {
    case "student":
      return "/student/dashboard";
    case "teacher":
      return "/teacher/dashboard";
    case "institute":
      return "/institute/dashboard";
    case "admin":
      return "/admin/dashboard";
    case "parent":
      return "/parent/dashboard";
    default:
      return "/profile";
  }
}

export function getRouteDisplayName(pathname, fallback = "") {
  const match = getKnownRouteMatch(pathname);
  if (match) return match.label;

  const path = normalizePath(pathname);
  const cleanedFallback =
    typeof fallback === "string"
      ? fallback.replace(/\s*\|\s*Learnova$/i, "").trim()
      : "";

  if (cleanedFallback) {
    return cleanedFallback;
  }

  if (!path || path === "/") return fallback || "Home";

  const lastSegment = path.split("/").filter(Boolean).pop() || "";
  const formatted = lastSegment
    .replace(/[-_]+/g, " ")
    .replace(/\b\w/g, (char) => char.toUpperCase())
    .trim();

  return formatted || fallback || "Page";
}

export function getRouteIcon(pathname) {
  const match = getKnownRouteMatch(pathname);
  return match?.icon || ArrowRight;
}

export function getSearchModalItems({ isAuthenticated, role } = {}) {
  const items = [
    { label: "Home", href: "/", category: "Navigation", icon: Home },
    {
      label: "Activities",
      href: "/activity",
      category: "Navigation",
      icon: Activity,
    },
    { label: "Contact", href: "/contact", category: "Navigation", icon: Mail },
  ];

  if (isAuthenticated) {
    items.push(
      {
        label: "Dashboard",
        href: getDashboardLink(role),
        category: "Account",
        icon: LayoutDashboard,
      },
      { label: "Profile", href: "/profile", category: "Account", icon: User },
      {
        label: "Settings",
        href: "/settings",
        category: "Account",
        icon: Settings,
      },
      {
        label: "Mark Attendance",
        href: "/attendance",
        category: "Quick Actions",
        icon: UserCheck,
      },
      {
        label: "Notice Board",
        href: "/notices",
        category: "Quick Actions",
        icon: Bell,
      }
    );
  } else {
    items.push({
      label: "Login / Signup",
      href: "/auth",
      category: "Account",
      icon: LogIn,
    });
  }

  return items;
}

...
describe("TransactionCoordinator", () => {
  ...
  describe("executeSaga", () => {
    ...
    it("allows retrying a failed operation by atomically transitioning its status", async () => {
      const duplicateError = new Error("E11000 duplicate key error");
      duplicateError.code = 11000;
      mockCollection.insertOne.mockRejectedValue(duplicateError);
      mockCollection.findOne.mockResolvedValue({ status: "failed" });
      ...
      const result = await executeSaga({
        ...
        uid: "user-1",
        idempotencyKey: "existing-key",
        steps: [{ name: "step1", execute: step1, compensate: vi.fn() }],
      });

      expect(result.success).toBe(true);
      expect(step1).toHaveBeenCalled();
      expect(mockCollection.updateOne).toHaveBeenCalledWith(
        { operationId: "existing-key", status: "failed" },
        expect.objectContaining({

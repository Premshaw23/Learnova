...
export async function awardXp(firebaseUid, actionType, metadata = {}) {
  ...
  while (attempt < MAX_RETRIES) {
      ...
      try {
        ...
        snapshot.forEach((doc) => {
          ...
          try {
            ...
            timeString = `${pad(dateObj.getHours())}:${pad(
              dateObj.getMinutes()
            )}`;
          }
    ...
    console.warn(
      `[awardXp] Concurrency collision detected for user ${firebaseUid} on attempt ${attempt}. Retrying...`
    );
  ...
  throw new Error(
    `Failed to award XP for user ${firebaseUid} after ${MAX_RETRIES} concurrent retry attempts.`
  );
}

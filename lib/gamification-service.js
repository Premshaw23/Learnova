 ...
 * Uses local date keys to prevent timezone-induced streak misclassification
 * (e.g., a check-in at 12:30 AM IST being compared against UTC dates).
...
function processStreak(student, now) {
  ...
  if (!student.lastDateKey) {
    currentStreak = 1;
  } else if (student.lastDateKey === todayKey) {
    currentStreak = previousStreak;
  } else {
    const lastDate = new Date(student.lastAttendanceDate);
    const lastKey = getLocalDateKey(lastDate);
    const expectedNextKey = getLocalDateKey(new Date(lastDate.getTime() + 86400000));

    if (todayKey === expectedNextKey) {
...
export async function awardXp(firebaseUid, actionType, metadata = {}) {
  ...
  while (attempt < MAX_RETRIES) {
    ...
    if (actionType === "attendance_marked") {
      ...
      updateFields.lastDateKey = getLocalDateKey(now);
    }

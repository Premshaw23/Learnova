...
describe("badgeEngine tests", () => {
  test("generates mock attendance records", () => {
    const perfectRecords = generateMockAttendanceRecords("perfect");
    expect(perfectRecords).toHaveLength(30);

    const earlyRecords = generateMockAttendanceRecords("early_bird");
    expect(earlyRecords).toHaveLength(10);
  });

  test("runs mock badge calculations", () => {
    const results = runAllTests();
    expect(results).not.toBeNull();
    expect(results.perfect.unlocked).toBe(true);
  });
});
